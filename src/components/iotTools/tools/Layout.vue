...
export default {
    ...
    computed: {
        layoutConfig: {
            get() {
                return this.$store.getters.get_activeTools;
            },
            set(data) {
                this.$store.commit("update_toolsList", data);
            }
        },
        activeToolsKey() {
            return this.$store.getters.get_activeToolsKey;
        }
    },
    methods: {
        // 在目的地
        ...
        drop(ev, index) {
            // 设置选中的 控件
            this.$store.commit("set_activeToolsKey", this.item.key);
            ...
            // 使用 JSON 转化 字符串 解决浅拷贝
            let options = JSON.parse(JSON.stringify(this.BaseTools[type]));
            options.key = key;
            let layoutConfig = JSON.parse(JSON.stringify(this.layoutConfig));
            layoutConfig.cols.splice(index, 1, options);
        ...
        delLayoutCol(index){
            ...
            // console.log(index);
            let layoutConfig = JSON.parse(JSON.stringify(this.layoutConfig));
            layoutConfig.cols.splice(index, 1);
            this.layoutConfig = layoutConfig;
        },
        // 选择 col
        selectLayoutCal(key){
            ...
            this.$store.commit("set_activeToolsKey", key);
        },
    }
};
</script>
