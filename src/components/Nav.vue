    ...
    <div :class="{
        ...
        >
            <!-- <template v-for="(v, k) in nav_list">
                <el-submenu v-if="v.children" :key="k" :index="v.link">
                    <template slot="title">
                ...
                <el-menu-item v-else :key="k" :index="v.link" :route="v.link">
                    <i :class="['iconfont', v.icon]"></i>
                    <span slot="title">{{v.name}}</span>
                </el-menu-item>
            </template> -->
...
export default {
    ...
    created(){
        ...
        this.nav_list = this.$router.options.routes;
    },
    computed: {
        nav_active(){
            return this.$store.getters.get_navActive;
        }
    },
    methods: {
        // 展开收起 导航菜单
